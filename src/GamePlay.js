...
GamePlayManager = {
    ...
    create: function() {
        ...
        this.totalTime = 15;
        this.timerText = game.add.text(1000, 15, this.totalTime+'', style);
        this.timerText.anchor.setTo(0.5);

        this.timerPERDIO = game.time.events.loop(Phaser.Timer.SECOND, function(){
            if(this.flagFirstMouseDown) {
                this.totalTime--;
                this.timerText.text = this.totalTime+'';
                if(this.totalTime <= 0){
                    game.time.events.remove(this.timerPERDIO);
                    this.showFinalMessage('PERDIO');
                    this.endGame = true;
                }
            }
        },this);
    ...
    increaseScore:function(){
        ...
        if(this.amountDiamondsCaught >= AMOUNT_DIAMONDS) {
            game.time.events.remove(this.timerPERDIO);
            this.showFinalMessage('FINALIZADO');
            this.endGame = true;
        }
    },
